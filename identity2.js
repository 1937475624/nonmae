...
chooseCharacterMcBossOL:function(){
var next=game.createEvent('chooseCharacter',false);
next.showConfig=true;
next.setContent(function(){
"step 0"
var num=lib.config.mc_BossList.Ai||1;
var Ai=[];
for(i=0;i<game.players.length;i++){
if(game.players[i]!=game.me&&!game.players[i].nickname&&num>0){
num--;
Ai.add(game.players[i]);
if(!game.zhu){
...
game.over('你TM不加人机打个屁Boss！！！');
}else{
for(i=0;i<game.players.length;i++){
game.players[i].send(function(player,zhu,Ai){
for(var i in lib.playerOL){
if(!Ai.contains(lib.playerOL[i])){
lib.playerOL[i].setIdentity('fan');
}else{
if(lib.playerOL[i]==zhu) zhu.setIdentity('zhu');
else lib.playerOL[i].setIdentity('zhong');
};
lib.playerOL[i].node.identity.classList.remove('guessing');
};
},game.players[i],game.zhu,Ai);
};
};
event.Ai=Ai;
...
var dialog=ui.create.characterDialog("请等待其他角色选将",'heightset',filter).open();
dialog.videoId=id;
ui.arena.classList.add('choose-character');
},list,event.videoId);
"step 2"
if(!event.current){
if(game.zhu){
event.current=game.zhu.next
}else event.current=game.players[0];
}else event.current=event.current.next;
if(event.current!=game.zhu&&(!event.current2||event.current2!=event.current)&&!event.Ai.contains(event.current)){
var fun=function(id,str){
var dialog=get.idDialog(id);
dialog.content.firstChild.innerHTML=str;
};
if(event.current==game.me) fun(event.videoId,"请选择武将");
if(event.current.isOnline2()) event.current.send(fun,event.videoId,"请选择武将");
event.current.chooseButton(true).set('ai',function(button){
return Math.random();
}).set('dialog',event.videoId).set('filterButton',function(button){
return _status.characterlist.contains(button.link);
});
};
if(event.current2&&event.current2==event.current) event.goto(4);
else if(event.Ai.contains(event.current)) event.redo();
"step 3"
if(!event.current2) event.current2=event.current;
if(event.current!=game.zhu){
var fun=function(id,str){
var dialog=get.idDialog(id);
dialog.content.firstChild.innerHTML=str;
};
if(event.current==game.me) fun(event.videoId,"请等待其他角色选将");
if(event.current.isOnline2()) event.current.send(fun,event.videoId,"请等待其他角色选将");
...
},event.current,result.links[0],event.videoId);
};
if(event.current!=game.zhu) event.goto(2);
"step 4"
...
ui.arena.classList.remove('choose-character');
			...
			if(lib.config.mc_hh&&lib.config.mc_Boss){
			game.chooseCharacterMcBossOL();
			return;
			};
				if(_status.mode=='purple'){
